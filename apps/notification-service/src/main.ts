...
async function bootstrap() {
  ...
  const notificationQueue = process.env.NOTIFICATION_RABBITMQ_QUEUE ?? 'notification_queue';
  const port = parseInt(process.env.NOTIFICATION_SERVICE_PORT ?? process.env.PORT ?? '3009', 10);
  ...
  logger.log(`   Queue : ${notificationQueue}`);
  logger.log('========================================');
}
