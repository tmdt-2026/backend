...
async function bootstrap() {
  ...
  const inventoryQueue = process.env.INVENTORY_RABBITMQ_QUEUE ?? 'inventory_queue';
  const port = parseInt(process.env.INVENTORY_SERVICE_PORT ?? process.env.PORT ?? '3003', 10);
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      ...
      prefetchCount: parseInt(process.env.INVENTORY_RABBITMQ_PREFETCH ?? '10', 10),
    },
  });
